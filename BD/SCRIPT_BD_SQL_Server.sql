--TABLE  T033_FORMA_FARMACEUTICA;
--DROP TABLE T034_CONCENTRACION;

---  FALTA LA PARTE DE PEDIDOS 


-- *** IMPORTANTE ....  CREAR EL USUARIO INTELJR Y ASIGNARLE LOS PRIVILEGIOS EN LA PARTE ROLES DEL SQLDEVELOPER OTORGAR TODO Y ADMINISTRAR TODO Y EN 
-- LA PARTE DE PRIIVILEGIOS DEL SISTEMA OTORGAR Y ADMINISTRAR TODO MENOS SYSDBA Y SYSOPER Y
-- AGREGARLE AL SCRIPT AL PRINCIPIO  EL CODIGO SIGUIENTE

   -- ALTER SESSION SET "_ORACLE_SCRIPT"= TRUE;
-- Y QUITAR KEPP SYSGUID  EN LA PARTE DE PRIVILEGIOS DE SISTEMA Y OTROS MAS

CREATE SEQUENCE  SQ_T058_PAIS_F058_CODIGO  MINVALUE 1 MAXVALUE 9999999 INCREMENT BY 1 START WITH 1 NOCACHE  ORDER  NOCYCLE ;
CREATE SEQUENCE  SQ_T059_DEPART_F059_CODIGO  MINVALUE 1 MAXVALUE 9999999 INCREMENT BY 1 START WITH 1 NOCACHE  ORDER  NOCYCLE ;
CREATE SEQUENCE  SQ_T060_CIUDAD_F060_CODIGO  MINVALUE 1 MAXVALUE 9999999 INCREMENT BY 1 START WITH 1 NOCACHE  ORDER  NOCYCLE ;



CREATE TABLE T002_TIPOS_IDTF (
  F002_CODIGO VARCHAR(3)  NOT NULL  ,
  F002_DESCRIPCION VARCHAR(15)  NOT NULL    ,
  F002_ACTIVO CHAR(1) CHECK (F002_ACTIVO IN ('S','N')),
PRIMARY KEY(F002_CODIGO));


INSERT INTO T002_TIPOS_IDTF VALUES('C.C','CEDULA CIUDADANO');
INSERT INTO T002_TIPOS_IDTF VALUES('NIT','ID EMPRESA');
INSERT INTO T002_TIPOS_IDTF VALUES('T.I','TARJETA IDENTIDAD');
INSERT INTO T002_TIPOS_IDTF VALUES('PSP','PASAPORTE');
...
SELECT * FROM T058_PAIS 
CREATE TABLE T058_PAIS(
F058_CODIGO  NUMERIC (3,0) IDENTITY(1,1) PRIMARY KEY ,
F058_DESCRIPCION VARCHAR(50),
F058_ACTIVO char(1)  CHECK  ( F058_ACTIVO IN ('N','S')) 
);

CREATE UNIQUE INDEX T058_F058_DESCRIPCION_IDX ON T058_PAIS(F058_DESCRIPCION); 
Insert into T058_PAIS(F058_DESCRIPCION,F058_ACTIVO)VALUES ('COLOMBIA','S')



CREATE TABLE T059_DEPARTAMENTOS
(
  F059_CODIGO NUMERIC(3,0 ) IDENTITY(1,1) PRIMARY KEY,
  F059_DESCRIPCION VARCHAR(50) ,
  F058_CODIGO NUMERIC(3,0),  --PAIS
  F059_ACTIVO char(1)  CHECK  ( F059_ACTIVO IN ('N','S')) ,
  CONSTRAINT FK_F058_CODIGO_T059  FOREIGN KEY (F058_CODIGO) REFERENCES T058_PAIS(F058_CODIGO) --PAIS
  );
  CREATE UNIQUE INDEX T059_F059_DESCRIPCION_IDX ON T059_DEPARTAMENTOS(F058_CODIGO,F059_DESCRIPCION);
 
 --- INSERT INTO    T059_DEPARTAMENTOS( F059_CODIGO,F059_DESCRIPCION,F058_CODIGO)  VALUES(0,'N/A',0) ;
  
  

  

CREATE TABLE T060_CIUDAD(
F060_CODIGO NUMERIC(3,0) IDENTITY(1,1)  PRIMARY KEY,
F060_DESCRIPCION VARCHAR(50),
F059_CODIGO NUMERIC(3,0),
F060_ACTIVO char(1)  CHECK  (F060_ACTIVO IN ('N','S')) 
CONSTRAINT   FK_F059_CODIGO_T060 FOREIGN KEY (F059_CODIGO) REFERENCES T059_DEPARTAMENTOS(F059_CODIGO)
); 
CREATE UNIQUE INDEX T060_F060_DESCRIPCION_IDX ON T060_CIUDAD(F059_CODIGO,F060_DESCRIPCION);
 -- INSERT INTO    T060_CIUDAD(F060_CODIGO,F060_DESCRIPCION,F059_CODIGO)  VALUES(0,'N/A',0) ;
  




CREATE TABLE T000_EMPRESAS(
 F000_CODIGO NUMERIC (3,0) PRIMARY KEY identity(1,1),  --F000_CODIGO
 F000_RAZON_SOCIAL VARCHAR2(50) NOT  NULL  , -- RAZON SOCIAL O NOMBRE DEL CLIENTE
 F000_DESCRIPCION VARCHAR(150), -- BREVE A QUE SE DEDICA LA EMPRESA
 
 F000_DIRECCION VARCHAR(60) NOT NULL  ,
 F000_TELEFONO VARCHAR(13) NOT NULL  ,
 F000_PAGINA_WEB VARCHAR(30)    NULL  ,
 F000_CORREO VARCHAR(25) NOT  NULL  ,  -- (DUEÑO EMPRESA)CORREO DEL USUARIO QUR CREA LA EMPRESA) SE DEBE INICIAR SESION PARA CREAR UNA EMPRESA
 
 --F000_REGIMEN CHAR(1) NOT  NULL   CHECK  ( F039_REGIMEN IN ('C','S')) ,
 F000_TIPO_PERSONA CHAR(1) NOT NULL  CHECK  ( F000_TIPO_PERSONA IN ('N','J')) ,
-- F000_ES_AUTORRETENEDOR CHAR(1) NOT NULL  ,
-- F000_EXCENTO_IVA CHAR(1)  NOT NULL  ,
 --F000_ES_GRANCONTRIBUYENTE CHAR(1) NOT NULL,
 F000_ACTIVO CHAR(1)  CHECK  (F000_ACTIVO IN ('N','S')) ,-- ACTIVA
 --OBLIGATORIO PAIS PRIMERO 
 F058_CODIGO   NUMERIC (3,0) NOT NULL, -- CODIGO DEL PAIS PRINCIPAL DE LA EMPRESA
 
-- CMAPOS OPCIONALES MOSTRAR DE ULRIJMO PARA DIDGITACION 
 F002_CODIGO NUMERIC(2,0)   NULL  ,  --  FK COD  TIPO DOCUMENTO(NIT,C.C, )
 F000_NRO_IDTF VARCHAR(15)    NULL  , -- NRO IDENTIFICACION DOCUMENTO     
 F002_CODIGO_PAIS_IDTF  NUMERIC (3,0) NULL,-- PAIS DE LA IDENTIFICACION  DELA EMPRESA
 F000_CORREO_SOPORTE VARCHAR  (25) NULL, -- CORREO PARA MOSTRAR
 
  CONSTRAINT FK_F002_CODIGO_T000  FOREIGN KEY  (F002_CODIGO) REFERENCES T002_TIPOS_IDTF(F002_CODIGO), -- TIPO DE DOCUMENTO
  CONSTRAINT FK_F058_CODIGO_T000  FOREIGN KEY  (F058_CODIGO) REFERENCES T058_PAIS(F058_CODIGO), -- CODIGO DEL PAIS PRINCIPAL DE LA EMPRESA
  CONSTRAINT FK_F000_CORREO_T000  FOREIGN KEY  (F000_CORREO) REFERENCES T013_USUARIO(F013_CORREO), -- CODIGO CORREO DUEÑO QUE CREO DE LA EMPRESA
);

-- RELACION EMPRESAS - USURIO  N - N
 CREATE TABLE T066_USU_EMPRE(
    F066_CODIGO NUMERIC(4,0) PRIMARY KEY IDENTITY(1,1),--ID AUTO
    F000_CODIGO NUMERIC (3,0),   --FK COD EMPRESA
	F013_CODIGO NUMERIC(6,0),     --FK COD USUARIO
	F066_FECHA_CREA DATETIME,
	CONSTRAINT FK_F000_CODIGO_T066   FOREIGN KEY  (F000_CODIGO)  REFERENCES T000_EMPRESAS(F000_CODIGO),  --   
	CONSTRAINT FK_F013_CODIGO_T066   FOREIGN KEY  (F013_CODIGO)  REFERENCES T013_USUARIO(F013_CODIGO)  --   
 );
   CREATE UNIQUE INDEX T066_F000COD_F013COD_IDX ON T066_USU_EMPRE(F000_CODIGO,F013_CODIGO);
 --*******   PENDIENTE   *****************
    --1. ROL SEGUN EMPRESA 
	  -- LA TABLA T066 ES UN IDENTIFICADOR DE USUARIO x EMPRESA  RELACIONARLA CON VARIOS ROLES QUE 
	  -- PUEDE TENER UN USUARIO
	    

   --2. BODEGAS DE USUAIO YA F011_CODIGO  NO VA EN LA TABLA T013_USUARIO PORQUE AHORA 
       -- EL USU SEGUN EMPRESA PUEDE TENER DIFERENTES BODEGAS
 --*******   PENDIENTE   *****************

--USUARIOS DEL SISTEMA
CREATE TABLE T013_USUARIO (
  F013_CODIGO NUMERIC(6,0)  NOT NULL PRIMARY KEY IDENTITY(1,1) ,  --PK  COD USUARIO
  F013_CORREO VARCHAR(60)  NOT NULL  , ----- UNICOOOOO es el login Y ES CON EL QUE SE BUSCA LAS EMPRESAS QUE HA CREADO 

 -- F011_CODIGO NUMERIC(5,0)  NOT NULL  ,  --    COD BODEGA NOOOOOOOO

  --SOLO EL ADMIN PUEDE EDITAR ESTOS DATOS PARA FLEXIBILIZAR SI UNA EMPRESA QUIERE TENE UNOS DATOS O NO
  F002_CODIGO NUMERIC(2,0)  NOT NULL  ,    --  FK COD  TIPO DOCUMENTO
  F013_NRO_IDTF VARCHAR(20)  NOT  NULL  ,
  --pAIS DE LA IDENTIICACION

  F013_NOMBRE VARCHAR(25)  NOT   NULL  , -- NOMBRE DEL USUARIO
  F013_APELLIDOS VARCHAR(30) NOT  NULL  ,
  --F013_LOGIN VARCHAR(15)    NOT NULL  ,  --- UNICOOOOO
  F013_PASWORD VARCHAR(30) NOT  NULL  ,      --PASWORD SE GUARDA ENCRIPTADO
  F013_SALT VARCHAR(50) NOT NULL,              --PARA ENCRIPTAR EL PASQORD
  F013_TELEFONO VARCHAR(60) NOT NULL    ,
  F013_ACTIVO CHAR(1)   CHECK  (F013_ACTIVO IN ('N','S'))
...
PRIMARY KEY(F013_CODIGO));
   ...
   -- ALTER TABLE T013_USUARIO   ADD CONSTRAINT FK_F050_CODIGO_T013   FOREIGN KEY  (F050_CODIGO)  REFERENCES T050_ROLE(F050_CODIGO);  --  GENERICO 
    ...
    CREATE UNIQUE INDEX T013_F013_CORREO_IDX ON T013_USUARIO(F013_CORREO);



-- ROLES DE USUARIO POR EMPRESSA
CREATE TABLE T067_USU_EMPRE_T050_ROL (
  F067_CODIGO NUMERIC(5,0) PRIMARY KEY IDENTITY(1,1),
  F066_CODIGO NUMERIC(4,0)  NOT NULL  , -- FK COD USUARIO x EMPRESA
  F050_CODIGO NUMERIC(3,0)  NOT NULL    , -- FK ROLES
  F067_ACTIVO CHAR(1)   CHECK  (F067_ACTIVO IN ('N','S'))
  );

  CREATE UNIQUE INDEX T067_F066_COD_F050_COD ON T067_USU_EMPRE_T050_ROL(F066_CODIGO,F050_CODIGO);
...
INSERT INTO T013_USUARIO (F013_CODIGO,F011_CODIGO,F002_CODIGO,F013_NRO_IDTF,F013_NOMBRE,F013_APELLIDOS,F013_LOGIN,F013_PASWORD,F013_CORREO,F013_TELEFONO)
 VALUES (1,1,2,92033351,'Omar', 'Castilla','OMAR77','OMARC77*','ING_JOSEMAR100@HOTMAIL.COM','3120000000');
...
-- PAISES QUE TIENE UNA EMPRESA
CREATE TABLE T065_EMPRESA_PAIS(
  T065_CODIGO NUMERIC(3,0) PRIMARY KEY IDENTITY (1,1),-- ID RELACION N -N
  F000_CODIGO NUMERIC (2,0) ,-- COD EMPRESA
  F058_CODIGO NUMERIC (3,0) --PAIS

);
CREATE UNIQUE INDEX T065_F000_F058_IDX ON T065_EMPRESA_PAIS(F000_CODIGO,F058_CODIGO); 



CREATE TABLE T001_SUCURSAL(
  F001_CODIGO  NUMERIC (2,0) PRIMARY KEY identity(1,1),  
  F001_NOMBRE_SUCRUSAL VARCHAR(60),  -- NOMBRE SUCURSAL
   
  F000_CODIGO NUMERIC(2,0), -- FK  COD EMPRESA
  F058_CODIGO  NUMERIC (3,0), -- PAIS 

  F060_CODIGO NUMERIC(3,0), ---- CIUDAD 
  F001_ACTIVO CHAR(1), -- ACTIVA('S','N')

  CONSTRAINT FK_F000_CODIGO_T001 FOREIGN KEY (F000_CODIGO) REFERENCES T000_EMPRESAS(F000_CODIGO) ,-- COD EMPRESA
  CONSTRAINT FK_F058_CODIGO_T001 FOREIGN KEY (F058_CODIGO) REFERENCES T058_PAIS(F058_CODIGO),-- PAIS
...
select * from  T034_CONCENTRACION


CREATE TABLE T034_CONCENTRACION (
  F034_CODIGO  VARCHAR(6)   NOT NULL  ,
  F034_DESCRIPCION VARCHAR(60)  NULL  ,
PRIMARY KEY(F034_CODIGO));

CREATE SEQUENCE  SQ_T034_CONCEN_F034_CODIGO  MINVALUE 1 MAXVALUE 9999999 INCREMENT BY 1 START WITH 1 NOCACHE  ORDER  NOCYCLE ;
CREATE UNIQUE INDEX T034_F034_DESCRIPCION_IDX ON T034_CONCENTRACION(F034_DESCRIPCION); 

INSERT INTO T034_CONCENTRACION  VALUES  ('0','N/A' );

CREATE TABLE T033_FORMA_FARMACEUTICA (
  F033_CODIGO VARCHAR2(8)   NOT NULL   ,
  F033_DESCRIPCION VARCHAR2(100)  NULL    ,  -- IDX UNIQUE
PRIMARY KEY(F033_CODIGO));

CREATE SEQUENCE  SQ_T033_FORMAF_F033_CODIGO  MINVALUE 1 MAXVALUE 9999999 INCREMENT BY 1 START WITH 1 NOCACHE  ORDER  NOCYCLE ;
CREATE UNIQUE INDEX T033_F033_DESCRIPCION_IDX ON T033_FORMA_FARMACEUTICA (F033_DESCRIPCION); 

INSERT INTO T033_FORMA_FARMACEUTICA  VALUES  ('0','N/A' );
  
  CREATE TABLE T031_GENERICO (
  
    F031_CODIGO  VARCHAR2(10)   NOT NULL ,  --PK
    F031_NOMBRE_GEN VARCHAR2(60) NOT NULL,  -- NOMBRE GENERICO
    F034_CODIGO  VARCHAR2(6)   NOT NULL  , --FK
    F033_CODIGO  VARCHAR2(8)  NOT NULL  ,  --FK
   -- F031_DESCRIPCION_FINAL  VARCHAR2(100)   NULL    ,
  PRIMARY KEY(F031_CODIGO));
 ...
 CREATE SEQUENCE  SQ_T031_GENERICO_F031_CODIGO  MINVALUE 1 MAXVALUE 9999999 INCREMENT BY 1 START WITH 1 NOCACHE  ORDER  NOCYCLE ; 
  ...
  CREATE UNIQUE INDEX T031_F031_NOMBRE_GEN_IDX ON T031_GENERICO (F031_NOMBRE_GEN); 
  ALTER TABLE  T031_GENERICO ADD CONSTRAINT FK_F033_CODIGO_T031   FOREIGN KEY  (F033_CODIGO)  REFERENCES T033_FORMA_FARMACEUTICA(F033_CODIGO);
  ALTER TABLE  T031_GENERICO ADD CONSTRAINT FK_F034_CODIGO_T031   FOREIGN KEY  (F034_CODIGO)  REFERENCES T034_CONCENTRACION(F034_CODIGO);
  
  COMMENT ON COLUMN T031_GENERICO.F033_CODIGO IS 'CODIGO DE LA FORMA FARMACEUTICA';
  COMMENT ON COLUMN T031_GENERICO.F034_CODIGO IS 'CODIGO  DE LA CONCENTRACION';
  
  INSERT INTO T031_GENERICO  VALUES  ('0','N/A' ,'0','0');
  

   --*************
     CREATE TABLE T021_UNIDAD_CONTENIDO (
      F021_CODIGO VARCHAR2(4)  NOT NULL  ,-- NO ES AUTOINCREMENTABLE
      F021_DESCRIPCION VARCHAR2(15)  NOT NULL    ,
    PRIMARY KEY(F021_CODIGO));
    
    CREATE SEQUENCE  SQ_T021_UND_CONTEN_F021_CODIGO  MINVALUE 1 MAXVALUE 9999999 INCREMENT BY 1 START WITH 1 NOCACHE  ORDER  NOCYCLE ; 
    ...
    --   UNIDAD DE MEDIDA  Y CONVERSION
      CREATE TABLE T024_UNIDAD_MEDIDA (
        F024_CODIGO VARCHAR2(3)  NOT NULL   ,  --Kg,Mt,Lt,UND  UNIDA NO HAY NECESIDAD DE CONVERTIR unidad de medida primaria
        F024_DESCRIPCION VARCHAR2(15) NOT  NULL    ,   --  IDX  UNIQUE   METRO, KILOGRAMO, MILILITRO
      PRIMARY KEY(F024_CODIGO));
      ...
      CREATE TABLE T022_CONVER_UND (
        F022_CODIGO VARCHAR2(3)  NOT NULL  , --(Cm,Gr,Mlt) UNIDA NO HAY NECESIDAD DE CONVERTIR
        F024_CODIGO VARCHAR2(3)  NOT NULL  , --FK UNIDAD DE MEDIDA    Cod DE UNIDAD DE MEDID EJEMPLO :  KG,MT,ML
        F022_DESCRIPCION VARCHAR2(15)  NOT NULL  ,  -- IDX UNICO    CENTIMETRO, MILILITRO, MILIMETRO
        F022_UND_EQUIVAL NUMERIC(5,0)  NOT NULL ,     --POR EJEMPLO PARA KG ACA IRA 1000 , PARA METRO 100, PARA MILITRO 1000  
      PRIMARY KEY(F022_CODIGO));
      --   UNIDAD DE MEDIDA  Y CONVERSION
      
    CREATE UNIQUE INDEX T022_F022_DESCRIPCION_IDX ON T022_CONVER_UND (F022_DESCRIPCION);
    ALTER TABLE  T022_CONVER_UND ADD CONSTRAINT FK_F024_CODIGO_T022   FOREIGN KEY  (F024_CODIGO)  REFERENCES T024_UNIDAD_MEDIDA(F024_CODIGO);    
...
--    ************* EMPAQUES ***************
   ...
   CREATE TABLE T020_EMPAQUES (
    F020_CODIGO NUMERIC(4,0)  NOT NULL  ,-- AUTO
    F020_DESCRIPCION VARCHAR2(40)   NOT   NULL    ,  -- IDX UNICO
  PRIMARY KEY(F020_CODIGO));
 ...
 INSERT INTO  T020_EMPAQUES (F020_CODIGO,F020_DESCRIPCION)  VALUES(1,'CAJA');
 INSERT INTO  T020_EMPAQUES (F020_CODIGO,F020_DESCRIPCION)  VALUES(2,'BULTO');
INSERT INTO  T020_EMPAQUES (F020_CODIGO,F020_DESCRIPCION)  VALUES(3,'SOBRE');


      
    CREATE TABLE T037_EMPAQUES_PRIMARIOS (
      F037_CODIGO NUMERIC(4,0)  NOT NULL  ,
      F020_CODIGO NUMERIC(4,0)  NOT NULL  ,
      F021_CODIGO VARCHAR2(4)  NOT NULL  ,
      F037_CANTIDAD_CONTENIDO_PRIM NUMERIC(7,0)  NOT NULL  ,
      F037_DESCRIP_FINAL_EMP_PRIM VARCHAR2(60)  NOT NULL  ,
      F037_UNIDAD_EMPAQUE NUMERIC(7,0)  NOT NULL   ,
   PRIMARY KEY(F037_CODIGO));
   ...
   CREATE SEQUENCE  SQ_T037_EMP_PRIMA_F037_CODIGO  MINVALUE 1 MAXVALUE 9999999 INCREMENT BY 1 START WITH 1 NOCACHE  ORDER  NOCYCLE ; 
      ...
      ALTER TABLE  T037_EMPAQUES_PRIMARIOS ADD CONSTRAINT FK_F020_CODIGO_T037   FOREIGN KEY  (F020_CODIGO)  REFERENCES T020_EMPAQUES(F020_CODIGO);
      ALTER TABLE  T037_EMPAQUES_PRIMARIOS ADD CONSTRAINT FK_F021_CODIGO_T037   FOREIGN KEY  (F021_CODIGO)  REFERENCES T021_UNIDAD_CONTENIDO(F021_CODIGO);
      ...
      INSERT INTO   T037_EMPAQUES_PRIMARIOS (F037_CODIGO,F020_CODIGO,F021_CODIGO, F037_CANTIDAD_CONTENIDO_PRIM, F037_DESCRIP_FINAL_EMP_PRIM,F037_UNIDAD_EMPAQUE )  VALUES  (0,0,'UND',1  ,'N/A',1 );
  ...
  /       
    ...
    CREATE TABLE T038_EMPAQUE_SECUNDARIO (
      F038_CODIGO NUMERIC(4)  NOT NULL  ,
      F020_CODIGO NUMERIC(4,0)  NOT NULL  ,  --FK  EJE: CAJA  TABLA EMPAQUES
      F038_CANTIDAD_EMPAQUES_PRIMAR NUMERIC(7,0)  NOT NULL  , --CANTIDAD DE EMPAQUES PRIMARIOS  QUE CONTIENE
      F038_DESCRIP_FINAL_EMP_SEC VARCHAR2(40)  NOT NULL    ,
   PRIMARY KEY(F038_CODIGO));
      ...
      ALTER TABLE  T038_EMPAQUE_SECUNDARIO ADD CONSTRAINT FK_F020_CODIGO_T038   FOREIGN KEY  (F020_CODIGO)  REFERENCES T020_EMPAQUES(F020_CODIGO);
      ...
      --INSERTO EMPAQUES SECUNDARIOS    N/A
            INSERT INTO   T038_EMPAQUE_SECUNDARIO (F038_CODIGO,F020_CODIGO,F038_CANTIDAD_EMPAQUES_PRIMAR,F038_DESCRIP_FINAL_EMP_SEC)  VALUES  (0,0,1,'N/A');
  ...
  /             
    ...
    CREATE TABLE T023_VIA_ADMINISTRACION (
      F023_CODIGO NUMERIC(4)  NOT NULL  ,
      F023_DESCRIPCION VARCHAR2(60) NOT  NULL    ,  --IDX  UNICO
    PRIMARY KEY(F023_CODIGO));
   ...
   INSERT INTO  T023_VIA_ADMINISTRACION VALUES  ('0','N/A'); 
    ...
    CREATE TABLE T032_LINEA (
      F032_CODIGO NUMERIC(4,0)  NOT NULL  ,  --  FALTA CREAR LA SECUENCIA
      F032_DESCRIPCION VARCHAR2(60)  NOT NULL    ,  -- IDX UNICO 
     PRIMARY KEY(F032_CODIGO));
    ...
    INSERT INTO  T032_LINEA VALUES  ('2','GENERICO'); 
     ...
     CREATE TABLE T027_LABORATORIO (
      F027_CODIGO NUMERIC(4,0)  NOT NULL   ,--FALTA CREAR LA SECUENCIA
      F027_NOMBRE_LAB VARCHAR2(60)  NULL  ,
      F027_DIRECCION VARCHAR2(100)  NULL  ,
      NIT VARCHAR2(20)  NULL    ,
    
    PRIMARY KEY(F027_CODIGO));
   ...
   CREATE OR REPLACE TRIGGER TGR_T027_LABORATORIO_BEFO
    BEFORE INSERT ON T027_LABORATORIO 
    FOR EACH ROW 
    BEGIN
       IF  :NEW.F027_CODIGO  IS NULL THEN
         SELECT SQ_T027_LABOT_F027_CODIGO.NEXTVAL INTO :NEW.F027_CODIGO FROM DUAL;
       END IF;
    END;
  /        
 
  --Inserta Laboratorio base
 INSERT INTO T027_LABORATORIO
(F027_CODIGO, F027_NOMBRE_LAB, F027_DIRECCION, NIT)
VALUES(0, 'N/A', 'N/A', '0');
...
---  GRUPOS  Y SUBGRUPOS
       ...
       CREATE TABLE T041_GRUPO (
        F041_CODIGO VARCHAR2(4)  NOT NULL   ,
        F041_DESCRIPCION VARCHAR2(60)  NOT NULL    ,
      PRIMARY KEY(F041_CODIGO));
     ...
     CREATE TABLE T042_SUBGRUPO1 (
      F042_CODIGO VARCHAR2(4)  NOT NULL  ,
      F041_CODIGO VARCHAR2(4)  NOT NULL  , -- FK  COIDGO DEL GRUPO'
      F042_DESCRIPCION VARCHAR2(60)  NOT NULL    ,
    PRIMARY KEY(F042_CODIGO));
      ...
      ALTER TABLE  T042_SUBGRUPO1 ADD CONSTRAINT FK_F041_CODIGO_T042 FOREIGN KEY  (F041_CODIGO)  REFERENCES T041_GRUPO (F041_CODIGO);
      ...
      F043_CODIGO VARCHAR2(6)  NOT NULL   , -- ' CODIGO DEL SUBGRUPO2'
       F042_CODIGO VARCHAR2(4)  NOT NULL  , --FK COIDGO DEL SUBGRUPO1'
      -- F041_CODIGO VARCHAR2(4)  NOT NULL  ,   --FK COIDGO DEL GRUPO'
      F043_DESCRIPCION VARCHAR2(60)  NOT NULL    ,  --  DECRIPCION  DEL  SUBGRUPO 2
    PRIMARY KEY(F043_CODIGO));
        ...
        ALTER TABLE  T043_SUBGRUPO2 ADD CONSTRAINT FK_F042_CODIGO_T043 FOREIGN KEY  (F042_CODIGO)  REFERENCES T042_SUBGRUPO1 (F042_CODIGO);
...
-----------------
   ...
   -----------  CATEGORIAS  ---------------------
    ...
    CREATE TABLE T030_CATEGORIA (
     F030_CODIGO NUMERIC(4,0)  NOT NULL   ,  ---PK
     F030_DESCRIPCION VARCHAR2(100)  NULL    ,
   PRIMARY KEY(F030_CODIGO));
  ...
  /
    ...
    CREATE TABLE T044_SUBCATEGORIA1 (
      F044_CODIGO NUMERIC(4,0)   NOT NULL  , --PK  Auto Incrementable  FUNCION  CRUD_MAESTRO_DETALLE DELPHI
        F030_CODIGO NUMERIC(4,0)  NOT NULL  ,  -- FK  CODIGO  DE LA CATEGORIA
      F044_DESCRIPCION VARCHAR2(100) NOT  NULL    ,
    PRIMARY KEY(F044_CODIGO));
    
    CREATE SEQUENCE  SQ_T044_SUBCATEG_F044_CODIGO  MINVALUE 1 MAXVALUE 9999999 INCREMENT BY 1 START WITH 1 NOCACHE  ORDER  NOCYCLE ; 
  ...
  /
      ...
      ALTER TABLE  T044_SUBCATEGORIA1 ADD CONSTRAINT FK_F030_CODIGO_T044 FOREIGN KEY  (F030_CODIGO)  REFERENCES T030_CATEGORIA(F030_CODIGO);
      COMMENT ON COLUMN T044_SUBCATEGORIA1.F030_CODIGO IS 'FK COIDGO LA  CATEGORIA';
      
      
      
      CREATE TABLE T045_SUBCATEGORIA2 (
        F045_CODIGO NUMERIC(4,0)  NOT NULL   ,
        F044_CODIGO NUMERIC(4,0)   NOT NULL  ,  --'FK COIDGO LA  SUBCATEGORIA1';
        F045_DESCRIPCION VARCHAR2(100)  NULL    ,
      PRIMARY KEY(F045_CODIGO));
  ...
  /
      ...
      ALTER TABLE  T045_SUBCATEGORIA2 ADD CONSTRAINT FK_F044_CODIGO_T045 FOREIGN KEY  (F044_CODIGO)  REFERENCES T044_SUBCATEGORIA1(F044_CODIGO);
      COMMENT ON COLUMN T045_SUBCATEGORIA2.F044_CODIGO IS 'FK COIDGO LA  SUBCATEGORIA1';
 
 
 CREATE TABLE T028_SUFIJOS_PRODUCTOS (
  F028_CODIGO VARCHAR2(4)  NOT NULL ,
  F028_DESCRIPCION VARCHAR2(30)  NULL ,
PRIMARY KEY(F028_CODIGO));
...
CREATE TABLE T046_Unidad_Medida_MX_Estand (
  F046_CODIGO NUMERIC(4,0)  NOT NULL ,
  F046_DESCRIPCION VARCHAR2(50)  NOT NULL    ,
PRIMARY KEY(F046_CODIGO));

   CREATE SEQUENCE  SQ_T046_UM_MX_Stnd_F046_CODIGO  MINVALUE 1 MAXVALUE 9999999 INCREMENT BY 1 START WITH 1 NOCACHE  ORDER  NOCYCLE ; 

   INSERT INTO  T046_Unidad_Medida_MX_Estand VALUES  (0,'N/A'); 


CREATE TABLE T052_MODOS_COMPRA_VENTA (
  F052_CODIGO VARCHAR2(3)  NOT NULL  ,
  F052_DESCRIPCION VARCHAR2(14)  NOT NULL    ,
PRIMARY KEY(F052_CODIGO));

INSERT INTO T052_MODOS_COMPRA_VENTA Values('UND','UNIDAD');
INSERT INTO T052_MODOS_COMPRA_VENTA Values('BLT','BLISTER');
INSERT INTO T052_MODOS_COMPRA_VENTA Values('EPR','EMP PRIMARIO');
INSERT INTO T052_MODOS_COMPRA_VENTA Values('EPS','EMP SECUNDARIO');


--insert  CHECK ((F004_MODOVENTA_X_DEFECTO  = 'UND')  or  (F004_MODOVENTA_X_DEFECTO  = 'BLT') OR  (F004_MODOVENTA_X_DEFECTO  = 'EPR')  OR  (F004_MODOVENTA_X_DEFECTO  = 'EPS')     ) ,

        
        CREATE TABLE T011_BODEGA (
          F011_CODIGO NUMERIC(5,0)  NOT NULL   ,   --PK
          F011_NOMBRE_BODEGA VARCHAR2(50)  NULL  ,
          DIRECCION VARCHAR2(70)  NULL  ,
          TELEFONO VARCHAR2(60)  NULL    ,
        PRIMARY KEY(F011_CODIGO));
                
         INSERT INTO T011_BODEGA VALUES(1,'BODEGA CENTRAL','','');       
  ...
  --- ***   SE TRASLADA DESPUES DE T048_DOCUMENTO       PORQUE HACE REFERENCIA A EL 
...
CREATE TABLE T004_PRODUCTO (
    F004_CODIGO VARCHAR2(10)  NOT NULL  ,  --PK
 --  BEGIN FK 
    F031_CODIGO VARCHAR2(10)  NOT NULL  ,  -- GENERICO 
    ...
    F024_CODIGO VARCHAR2(3)  NOT NULL  ,  --  UNIDAD DE MEDIDA
    F023_CODIGO NUMERIC(4,0)  NOT NULL ,    --VIA ADMINISTRACION
    F041_CODIGO VARCHAR2(4)  NOT NULL  ,  --  GRUPO
    F042_CODIGO VARCHAR2(4)  NOT NULL  ,  --  SUBGRUPO
    F043_CODIGO VARCHAR2(6)  NOT NULL  ,  --  SUBGRUPO2
    ...
    F046_CODIGO NUMERIC(4,0)  NOT NULL,      -- UNIDAD DE MEDICAMENTO ESTANDAR MX
    F052_CODIGO VARCHAR2(3) NOT  NULL ,-- FK CHECK ((F004_MODOVENTA_X_DEFECTO  = 'UND')  or  (F004_MODOVENTA_X_DEFECTO  = 'BLT') OR  (F004_MODOVENTA_X_DEFECTO  = 'EPR')  OR  (F004_MODOVENTA_X_DEFECTO  = 'EPS')     ) ,
 -- END  FK
  ...
  F004_NOMBRE_COMERCIAL  VARCHAR2(100) NOT NULL,
  F004_DESCRIPCION VARCHAR2(300)  NOT NULL  ,
  F004_CODIGOBARRA VARCHAR2(20)   NULL  ,--- VALIDAR SI SE DEJA NULL
  
  F004_PRECIO_UNIT NUMERIC(18,3)   NULL  ,
  ...
  F004_PORC_IVA NUMERIC(5,2) NOT   NULL  , -- PORCENTAJE  DE IVA VENTA DEL PRODUCTO PERO AL VENDER SE TIENE EN CUENTA ES EL DE LA LISTA SELECCIONADA
  ...
  F004_POSOLOGIA VARCHAR2(100)  NULL  ,  ----        VALIDAR SI SE DEJA NULL
 ...
 -- F004_DIAS_MINIMO NUMERIC(4,0) NOT NULL  ,
  ...
  F004_CODIGO_MIN_SALUD VARCHAR2(20)  NULL  ,  -- PUEDE SER NULO
  
  F004_REGISTRO_INVIMA VARCHAR2(35)   NULL  ,  -- SI NO ES MEDICAMENTO  PUEDE SER NULL
  F004_CUM VARCHAR2(20)   NULL  ,-- SI NO ES MEDICAMENTO  PUEDE SER NULL
...
PRIMARY KEY(F004_CODIGO));
      ...
      CREATE SEQUENCE  SQ_T004_PRODUCTOS_F004_CODIGO  MINVALUE 1000 MAXVALUE 99999999999999999999999999 INCREMENT BY 1 START WITH 1000 NOCACHE  ORDER  NOCYCLE ;
    ...
    COMMENT ON COLUMN INTELJR.T004_PRODUCTO.F004_TIPO IS ' ''I'' = iNVENTARIO, ''S'' = SERVICIO, ''N''= PRODUCTO QUE NO AFECTA INVENTARIO.';
    ...
    COMMENT ON COLUMN  T004_PRODUCTO.F004_COSTO_UNIT_PROM_PONDERADO  IS  'COSTO PROMEDIO PONDERADO, QUE RESULTA DE SUMAR LOS COSTOS DE COMPRA DEL PRODUCTO  Y DIVIDIRLOS ENTRE EL TOTAL DE EXISTENCIAS.
SI EL PRODUCTO TIENE MARCADO EL CAMPO IVA_DESCONTABLE EN ''S'' ENTONCES  EL IVA NO SE INCLUYE EN EL COSTO PROMEDIO PONDERADO, DE LO CONTRARIO SI ESTA EN ''N'' SI SE INCLUYE PORQUE ESTE NO NO ES DESCONTABLE.';
   COMMENT ON COLUMN INTELJR.T004_PRODUCTO.F004_COSTOTOTAL_POMD_PONDE IS 'VALORIZACION TOTAL DEL COSTO PONDERADO DE ESTE PRODUCTO EN TODAS LAS BODEGAS DE LA EMPRESA.';
   ...
   -- COMMENT ON COLUMN  T004_PRODUCTO.F004_DIAS_MINIMO  IS  'DIAS MINIMO EN INVENTARIIO FORMULA => vendimos 60 pzas en 30 d�as vamos a tener que vendemos en promedio 2 pzas por d�a, y tenemos que el proveedor se tarda en resurtir 15 d�as, sabemos entonces nuestro m�nimo debe de ser 30 piezas en inventario.';
   --COMMENT ON COLUMN  T004_PRODUCTO.F004_DIAS_MAXIMO  IS  'DIAS MAXIMO EN INVENTARIIO  =>M�ximo de existencia = M�nimo + D�as de Gracia .     ==> D�as de Gracia = (Pzas promedio de venta por d�a) * (D�as que podemos soportar en bodega)';
   
     ALTER TABLE INTELJR.T004_PRODUCTO ADD F004_IVA_ES_DESCONTABLE CHAR(1);
     COMMENT ON COLUMN INTELJR.T004_PRODUCTO.F004_IVA_ES_DESCONTABLE IS 'Si es  ''S'' entonces en el calculo del promedio ponderado no se incluye, pues es decontable , pero si se marca como ''N'' se tiene en cuenta.(Trigger TGR_T049_DET_DOCUMENTO)';
     ...
     CREATE OR REPLACE TRIGGER TGR_T004_PRODUCTO
            ...
            BEGIN
              ...
              IF  (:NEW.F004_APLICA_BLISTER   = 'S')  AND  ((NVL(:NEW.F004_NRO_BLISTER,-1) <= 0) OR  (NVL(:NEW.F004_CANTIDAD_X_BLISTER,-1) <= 0) ) THEN
                RAISE_APPLICATION_ERROR(-20000, 'error: APLICA BLISTER, PERO EL N�MERO O LA CANTIDAD DE BLISTER POR EMPAQUE DEBEN SER MAYORES DE CERO(0).');
              END IF;
            END; 
  /
...
CREATE TABLE T026_TIPOS_MOVIMIENTO (
  F026_CODIGO VARCHAR2 (3)  NOT NULL ,  --(C  = ,COMPRA ; F  = FACTURA    .......)
  F026_CONCEPTO VARCHAR2(60) NOT   NULL  ,
  F026_TIPO_MOV VARCHAR(3) NOT  NULL     CHECK  (F026_TIPO_MOV  IN ('E','S','N/A')) ,
  F026_CAPTURAR VARCHAR2(9)  NOT NULL  ,
  F026_SGTE_FOLIO VARCHAR2(5)  NULL    ,
...
CREATE TABLE T012_TIPOS_DOCUMENTOS (
  F012_TIPO_DOC VARCHAR2(4)  NOT NULL ,
  F026_CODIGO VARCHAR2(3)  NOT NULL  , --FK (C  = ,COMPRA ; F  = FACTURA)
  F012_DESCRIPCION VARCHAR2(50)not   NULL    ,
...
ALTER TABLE T012_TIPOS_DOCUMENTOS   ADD CONSTRAINT FK_F026_CODIGO_T012   FOREIGN KEY  (F026_CODIGO)  REFERENCES T026_TIPOS_MOVIMIENTO(F026_CODIGO); --  SUBCATEGOROIA DE  NIVEL 2
 ...
 CREATE OR REPLACE TRIGGER TGR_T012_TIPO_DOC_BEFORE_UPD
    BEFORE UPDATE ON T012_TIPOS_DOCUMENTOS
    FOR EACH ROW 
 BEGIN
   IF  (:OLD.F026_CODIGO  <>  :NEW.F026_CODIGO)  OR  (:OLD.F012_TIPO_DOC <> :NEW.F012_TIPO_DOC) THEN
     RAISE_APPLICATION_ERROR(-20000,'Error: El tipo de documento ni el Tipo de Movimiento se puede editar.');
   END IF;
 END;
  /  

 CREATE OR REPLACE PROCEDURE SP_CRUD_SECUENCIA_DOC (SECUENCIA IN  VARCHAR2  )
    is
 ...
 CREATE OR REPLACE TRIGGER TGR_T012_TIPO_DOC_BEFORE
    ...
    declare EXISTE  numeric(2,0);
     CMD VARCHAR2(110);
    BEGIN
       SELECT count(*) into EXISTE from all_sequences   WHERE SEQUENCE_NAME    =  'SQ_T012_' || :NEW.F012_TIPO_DOC ; --  VALIDA QUE LA SECUENCIA( TIP� DE M) NO EXISTA
 ...
 */   
  ...
  INSERT INTO T012_TIPOS_DOCUMENTOS  VALUES('COMP','CO', 'COMPRAS');
  INSERT INTO T012_TIPOS_DOCUMENTOS  VALUES('ENCO','EO', 'ENTRADA CON ORDEN DE COMPRA');
  INSERT INTO T012_TIPOS_DOCUMENTOS  VALUES('_PUB','FV', 'VENTAS PUBLICO');
  INSERT INTO T012_TIPOS_DOCUMENTOS  VALUES('_FSP','FS', 'FACT SERVICIO PUB');
  --NOTAS DE DEVILUCION 
  INSERT INTO T012_TIPOS_DOCUMENTOS  VALUES('NDV','NDV', 'NOTA DEVOLUCION FISICA VENTA'); -- SI ENTRA A INVENTARIO 
  INSERT INTO T012_TIPOS_DOCUMENTOS  VALUES('DVP','DVP', 'NOTA DEVOLUCION VENTA POR PERDIDA'); -- NO ENTRA A IINVENTARIO
...
CREATE TABLE T039_CLIENTE (
  ...
  F039_RAZON_SOCIAL VARCHAR2(100) NOT  NULL  , -- RAZON SOCIAL O NOMBRE DEL CLIENTE
  
  F039_DIRECCION VARCHAR2(80) NOT NULL  ,
  F039_TELEFONO VARCHAR2(60) NOT NULL  ,
  F039_PAGINA_WEB VARCHAR2(60) NOT   NULL  ,
  F039_CORREO VARCHAR2(60) NOT  NULL  ,
  ...
  F039_ES_GRANCONTRIBUYENTE CHAR(1) NOT NULL,
...
CREATE TABLE  T057_BODEGAS_CLIENTE(
  ...
  -- PENDIENTE RESTRINGIR QUE NO SE INGRESE OTRA BODEGA CON NOMBRE => 'BODEGA PUBLICA'
CREATE SEQUENCE  SQ_T057_BOD_CLIENT_F057_COD MINVALUE 1 MAXVALUE 9999999 INCREMENT BY 1 START WITH 2 NOCACHE  ORDER  NOCYCLE ; 
  ...
  INSERT INTO T057_BODEGAS_CLIENTE(F057_CODIGO,F039_CODIGO,F057_DESCRIPCION,F058_CODIGO,F059_CODIGO,F060_CODIGO) VALUES (1,1,'BODEGA PUBLICA',0,0,0) ;  
    
   
   --SE TRASLADA DESPUES DE �A TABLA T048_DOCUMENTO  PORQUE HACE REFERENCIA A ELLA
...
CREATE TABLE T007_PROVEEDOR (F007_CODIGO NUMERIC(5,0)  NOT NULL   ,
  F007_NIT VARCHAR2(15)  NOT NULL  ,
  F007_RAZON_SOCIAL VARCHAR2(100) NOT  NULL  ,
  F007_TELEFONO VARCHAR2(20) NOT  NULL  ,
  F007_DIRECCION VARCHAR2(50) NOT NULL  ,
  F007_PAGINA_WEB VARCHAR2(60)   NULL  ,
  F007_CORREO VARCHAR2(60) NOT  NULL  ,
...
CREATE TABLE T051_ESTADOS_COMPRA (
  ...
  F051_DESCRICPCION VARCHAR2(10)  NOT NULL    ,
...
CREATE UNIQUE INDEX T047_BODEGA_PROD_LOTE_IDX ON T047_STOCK_LOTES(F011_CODIGO,F004_CODIGO, F047_LOTE); 
 ...
 ALTER TABLE T047_STOCK_LOTES   ADD CONSTRAINT FK_F004_CODIGO_T047 FOREIGN KEY  (F004_CODIGO)  REFERENCES T004_PRODUCTO (F004_CODIGO);  --  GENERICO 
  ...
  CREATE TABLE T062_DETLISTA_CLIE (
    ...
    F004_CODIGO VARCHAR2(10) NOT NULL, -- FK PRODUCTO
    F052_CODIGO VARCHAR2(3),  --FK  MODO DE VENTA POR DEFECTO (UND , BLISTER ,EMP PRIMARIO, EMP SECUNDARIO)
    ...
    CREATE SEQUENCE  SQ_T048_DOCUMENTO_F048_CODIGO  MINVALUE 1 INCREMENT BY 1 START WITH 1 NOCACHE  ORDER  NOCYCLE ; 
     ...
     CREATE OR REPLACE TRIGGER TGR_T048_DOCUMENTO
       ...
       CREATE OR REPLACE TRIGGER TGR_T048_DOCUMENTO_UPDATE
            ...
            DECLARE
            PCOD_MOV  VARCHAR2(3);
            COD_MOV_ID_RELACIONADO VARCHAR2(3);
...
-- SE COLOCA AQUI PORQUE HACE REFERENCIA A LAL TABLA T048_DOCUMENTO
  CREATE OR REPLACE TRIGGER TGR_T057_DELETE
    BEFORE DELETE ON T057_BODEGAS_CLIENTE 
    FOR EACH ROW
    DECLARE EXISTE_BOD_CLIENTE   NUMERIC(2,0);
...
CREATE TABLE T049_DET_DOCUMENTO (
    ...
    F004_CODIGO VARCHAR2(10)  NOT NULL  ,  --  FK PRODUCTO
    F052_CODIGO  VARCHAR2(3)  NOT NULL,   -- FK MODOCOMPRA / VENTA    CHECK  (F049_MODOCOMPRA  in ('UND','EPR','EPS')),  -- venta in ('UND','EPR','EPS,BLT'))
   ...
   -- F049_PRECIO_UNIT_MODO_VENTA NUMERIC (14,2),  -- SE TRAE EL PRECIO  SEGUN EL MODO DE VENTA(CON LOS 3 DECIMALES ) PUBLICO O DE LA LISTA SE REDONDEA Y   SE GUARDAN 2 DECIMALES ...  CALCULO(EN VENTA SEGUN MODO VENTA : UND (SE ASIGNA PRECIO X UNIDAD)   -   BLISTER(SE DIVIDE    PRECIO DEL BLISTER/EL TOTAL UND DEL BLISTER )   ---  EMPAQUE PRIM Y SEC( SE DIVIDE ASI :   CANTIDAD TOTAL DE UNIDADES DEL EMPAQUE   /  PRECIO_EMP_PRIMARIO ) ) .... **************       (  PARA COMPRAS SIEMPRE SE INGRESA EL PRECIO POR UNIDAD DIVIDIENDO =>    TOTAL ITEM   / EL TOTAL DE UNIDADES  )                            LO TRAE DE LA LISTA DE PRECIOS DEL PROVEEDOR
    ...
    F049_PORC_IVA_ULTIMA_COMPRA NUMERIC(4,2), 
	F049_COSTO_UNIT_PROM_PONDERADO_ULTIMO NUMERIC(14,2), 
	F049_COSTOTOTAL_POMD_PONDE_ULTIMO NUMERIC(18,2), 
	 CHECK (F049_MANEJA_INVENTARIO IN  ('S','N')) ENABLE, 
  ...
  COMMENT ON COLUMN  T049_DET_DOCUMENTO.F049_COSTO_ULTIMO IS 'AQUI SE GUARDA EL F004_COSTO_UNIT_ULTIMA_COMPRA  QUE ES ELCAMPO QUE CONSERVA EL COSTO DE LA ULTIMA COMPRA DE DICHO PRODUCTO REALIZADA EN EL SISTEMA';
  COMMENT ON COLUMN INTELJR.T049_DET_DOCUMENTO.F049_PORC_IVA_ULTIMA_COMPRA IS '%IVA RELACIONADO CON    F049_COSTO_ULTIMO QUE ES DONDE SE ESTA GUARDANDO EL COSTO DE LA ULTIMA COMPRA ALMOMENTO DE REAIZAR LA VENTA.';
  COMMENT ON COLUMN INTELJR.T049_DET_DOCUMENTO.F049_COSTO_UNIT_PROM_PONDERADO_ULTIMO IS 'COSTO UNITARIO PROMEDIO PONDERADO   QUE ES DONDE SE ESTA GUARDANDO EL COSTO UNITARIO  PONDERADO  QUE ESTABA 
 AL MOMENTO DE REAIZAR LA VENTA.';
       ...
       CREATE OR REPLACE TRIGGER TGR_T049_DET_DOCUMENTO
            ...
            DECLARE TIPO CHAR(1);
                    ...
                    EXISTENCIAS_INVENTPROM NUMERIC(15,2);
                    COSTOUNIT_PROMPOND NUMERIC(14,2);
                  

                    COD_MOV  VARCHAR(3); --- DEL DOCUMENTO QUE SE ESTA PROCEANDO NO DEL RELACIONADO
                    PF048_ID_RELACIONADO NUMERIC(12,0);
                    PROD_IVA_ES_DESCONTABLE CHAR(1); -- 'S' O 'N' Si es S no se incluye en el costo ponderado, sino si se incluye.
            ...
            BEGIN
              ...
              END IF;
                     ...
                     select F004_DESCRIPCION ,F004_TIPO,F004_REQUIERE_LOTE,F004_REQUIERE_UBICACION, F004_IVA_ES_DESCONTABLE INTO DESCRIPCION_PROD,TIPO, REQUIERE_LOTE, REQUIERE_UBICACION,PROD_IVA_ES_DESCONTABLE  from T004_PRODUCTO   PRO
                     WHERE PRO.F004_CODIGO = :NEW.F004_CODIGO   ;


                       --  PF048_ID_RELACIONADO, BODEGA(DOC)  Y  TIPO DE MOVIMIENTO DEL DOCUMENTO => E = ENTRADA  S = SALIDA   N/A   =  NO APLICA
                   ...
                   -- IF  COD_MOV  = 'FS'  then   -- EL DOCUMENTO ES FACTURA DE SERVICIO 
					...
					IF TIPO_MOVI_DOC ='N/A' THEN -- COUEMENTO NO AFECTA INVENTARIO
    ...
    IF TIPO  = 'I'  THEN -- VALIDA INVENTARIO SOLO PARA LOS PRODUCTOS MAS NO PARA LOS SERVICIOS
                                              ...
                                              IF REQUIERE_LOTE   = 'S'  THEN
                                                   ...
                                                   -- EXISTE PRODUCTO Y LOTE EN LA TABLA DE STOCK  PARA LA BODEGA DE ESTE DOCUMENTO
                                                     ...
                                                     ELSE  --  SINO EXISTE EL LOTE INSERTE EL NUEVO LOTE CON SUS CANTIDADES
                                                      RAISE_APPLICATION_ERROR(-20019,'EL LOTE DEL PRODUCTO ' || DESCRIPCION_PROD || ' NO �XISTE , INDIQUE UN LOTE CORRECTO -  O EL PRODUCTO NO SE LE HA HECHO NINGUN INGRESO' );  
                                                      END IF;
                           ...
                           IF TIPO_MOVI_DOC   = 'S'  then  --  SALIDA DE ALMACEN   
                                       ...
                                       -- SI REQUIERE LOTE SI YA EXISTE SINO INSERTELO
                                              IF REQUIERE_LOTE   = 'S'  THEN
                                                   ...
                                                   -- EXISTE PRODUCTO Y LOTE EN LA TABLA DE STOCK  PARA LA BODEGA DE ESTE DOCUMENTO
                                                      select COUNT(*)  INTO EXISTEPROD_T047_STOCK_LOTE from T047_STOCK_LOTES   STOCK
                                                      WHERE STOCK.F004_CODIGO = :NEW.F004_CODIGO AND STOCK.F011_CODIGO  = BODEGA  AND   
													         STOCK.F047_LOTE  = :NEW.F049_LOTE_SERIAL  ;
                                            ...
                                            --*** ACTUALIZO PROMEDIO PONDERADO
                                            
													-- TOTAL DE EXISTENCIAS EN INVENTARIO DEL PRODUCTO
	                                              SELECT  SUM(STOCK.F047_EXISTENCIAS)  INTO  EXISTENCIAS_INVENTPROM from T047_STOCK_LOTES STOCK
		                                          WHERE  F004_CODIGO = :NEW.F004_CODIGO
		          							      GROUP BY STOCK.F004_CODIGO;
	                                            
	                                                UPDATE T004_PRODUCTO SET F004_COSTOTOTAL_POMD_PONDE  = F004_COSTO_UNIT_PROM_PONDERADO  * EXISTENCIAS_INVENTPROM
						                            WHERE  F004_CODIGO = :NEW.F004_CODIGO; 
						                    --*** END... ACTUALIZO PROMEDIO PONDERADO       
                                       ...
                                       -- SI REQUIERE LOTE SI YA EXISTE SINO INSERTELO
                                              IF REQUIERE_LOTE   = 'S'  THEN
                                                   ...
                                                   -- EXISTE PRODUCTO Y LOTE EN LA TABLA DE STOCK  PARA LA BODEGA DE ESTE DOCUMENTO
                                                     ...
                                                     ELSE  --  SINO EXISTE EL LOTE INSERTE EL NUEVO LOTE CON SUS CANTIDADES
                                                       RAISE_APPLICATION_ERROR(-20019,'EL LOTE DEL PRODUCTO ' || DESCRIPCION_PROD || ' NO �XISTE , INDIQUE UN LOTE CORRECTO O CREELO' );  
                                                                   /*INSERT INTO   
                                                                      --F047_CODIGO 
                                                                     (F011_CODIGO ,
                                                                      F004_CODIGO ,
                                           ...
                                           --*** PROMDEIO PONDERADO **** AQUI YA SE HA AGREGDO AL INVENTARIO EL PROD  EN LA TRANSACCION ACTUAL 
                                             
                                              -- SI ES UNA ENTRADA CON ORDEN DE COMPRAS  (pENDIENTE VALIDAR REMISION )
                                              IF COD_MOV  =  'EO'  /* OR 'INVINICIAL'  OR 'REMISION' */  THEN  
	                                              
                                              	-- TOTAL EXISTENCIA EN STOCK DE UN PRODUCTO (INDEPENDIENTE DEL LOTE SE TOTALIZA TODO)
	                                                SELECT  SUM(STOCK.F047_EXISTENCIAS)  INTO  EXISTENCIAS_INVENTPROM from T047_STOCK_LOTES STOCK
	                                                WHERE  F004_CODIGO = :NEW.F004_CODIGO
	          										GROUP BY STOCK.F004_CODIGO;
          									

                                            --  --*** PROMDEIO PONDERADO ****  (pENDIENTE VALIDAR REMISION  E IVA DESCONTABLE)   COSTO ULTIMA COMPRA
		                                                  
			          								IF EXISTENCIAS_INVENTPROM > 0 THEN	 
	          									          IF PROD_IVA_ES_DESCONTABLE  <> 'S' THEN -- Si iva no es descontable INCLUYA IVA EN EL COSTO PONDERADO EN T004
					                                              UPDATE T004_PRODUCTO 
					                                              SET
					                                                     F004_COSTO_UNIT_PROM_PONDERADO = ROUND(((:NEW.F049_VALOR_NETO_ITEM + NVL(F004_COSTOTOTAL_POMD_PONDE,0) )/ EXISTENCIAS_INVENTPROM),2),
					                                                     F004_COSTOTOTAL_POMD_PONDE = ROUND(((:NEW.F049_VALOR_NETO_ITEM + NVL(F004_COSTOTOTAL_POMD_PONDE,0) )/ EXISTENCIAS_INVENTPROM),2) * EXISTENCIAS_INVENTPROM
					                                              WHERE  F004_CODIGO = :NEW.F004_CODIGO;  
					                                         ELSE --IVA SI ES DESCONTABLE RESTELO  NO LO INCLUYA EN EL COSTO  PROMEDIO PONDERADO
					                                            -- EL IVA NOOO  HACE PARTE DEL COSTO PUES ES UN IVA DESCONTABLE CUANDO SE REALIZA LA VENTA
					                                             UPDATE T004_PRODUCTO 
					                                             SET 
					                                                     F004_COSTO_UNIT_PROM_PONDERADO = ROUND( (( (:NEW.F049_VALOR_NETO_ITEM - :NEW.F049_VALOR_IVA_ITEM) + NVL(F004_COSTOTOTAL_POMD_PONDE,0) )/ EXISTENCIAS_INVENTPROM),2),
					                                                     F004_COSTOTOTAL_POMD_PONDE = ROUND((( (:NEW.F049_VALOR_NETO_ITEM - :NEW.F049_VALOR_IVA_ITEM) + NVL(F004_COSTOTOTAL_POMD_PONDE,0) )/ EXISTENCIAS_INVENTPROM),2) * EXISTENCIAS_INVENTPROM
					                                              WHERE  F004_CODIGO = :NEW.F004_CODIGO;
					                                         END IF; -- NVL(F049_PORC_IVA,0)  = 0  
					                                ELSE  --EXISTENCIAS_INVENTPROM = 0
					                                
				                                             UPDATE T004_PRODUCTO 
				                                             SET
				                                                    F004_COSTO_UNIT_PROM_PONDERADO = 0,
				                                                    F004_COSTOTOTAL_POMD_PONDE = 0
				                                              WHERE  F004_CODIGO = :NEW.F004_CODIGO;
					                                END IF; --EXISTENCIAS_INVENTPROM > 0  
					                                
					                             --END *** PROMDEIO PONDERADO ****    
					                                
					                                        --- ** COSTO_UNIT_ULTIMA_COMPRA
					                                		IF PROD_IVA_ES_DESCONTABLE  <> 'S' THEN -- Si iva no es descontable INCLUYA IVA EN EL COSTO  ULTIMA COMPRA
					                                              ...
					                                              WHERE  F004_CODIGO = :NEW.F004_CODIGO;  
					                                         ELSE  --IVA SI ES DESCONTABLE RESTELO  NO LO INCLUYA EN EL COSTO  ULTIMA COMPRA
					                                            -- EL IVA NOOO  HACE PARTE DEL COSTO PUES ES UN IVA DESCONTABLE CUANDO SE REALIZA LA VENTA
					                                             UPDATE T004_PRODUCTO SET F004_COSTO_UNIT_ULTIMA_COMPRA  = ROUND(( (:NEW.F049_VALOR_NETO_ITEM - :NEW.F049_VALOR_IVA_ITEM)/ :NEW.F049_CANTIDAD_TOTAL_INVENTARIO),2) 
					                                                    WHERE  F004_CODIGO = :NEW.F004_CODIGO;
					                                         END IF; -- NVL(F049_PORC_IVA,0)  = 0  
					                                         
					                                         --*** % IVA ULTIMA COMPRA
					                                         UPDATE T004_PRODUCTO SET F004_PORC_IVA_ULTIMA_COMPRA  = :NEW.F049_PORC_IVA 
					                                              WHERE  F004_CODIGO = :NEW.F004_CODIGO; 
                                              ...
                                              END IF; --COD_MOV  =  'EO'
                                               ...
                                               --  OJOOOOOOOOOOOOOOOOOO AQUI EMPIEZAN LAS VALIDACIONES PARA DESCONTAR DEL DOCUMENTO RELACIONADO                                      
                                                ...
                                                --nO SE PUEDE ESTO ACA DA ERROR TRIGGER MUTANTE SE PASO A LA FORMA ENTRADA CON ORDEN DE COMPRA. 
                                                /* IF COD_MOV  =  'EO'  THEN  -- SI ES UNA ENTRADA CON ORDEN DE COMPRAS SIN IMPORTAR EL TIPO DE DOCUMENTO RELACIONADO ( ORDEN DE COMPRA O REMISION)
                                                     -- RAISE_APPLICATION_ERROR(-20020,'ERROR: EL   XXXX');
                                                    -- ACTUALIZA LA ORDEN DE COMPRA O REMISION                                                                   
                                                     UPDATE T049_DET_DOCUMENTO  SET F049_CANTIDAD_PTE_TOTAL_INVENT  = F049_CANTIDAD_PTE_TOTAL_INVENT - :NEW.F049_CANTIDAD_TOTAL_INVENTARIO  -- SI F049_CANTIDAD_PTE_TOTAL_INVENT ES NEGEATIVO  ESTE VALOR EN LA ORDEN DE COOMPRA ES PORQUE SE ENTRARON MAS CANTIDADES DE LAS QUE SE PIDIO
                                               ...
                                               --IF ES UNA ENTRADA CON ORDEN DE COMPRA DESCUENTE LAS CANTIDADES DE UNIDADES TOTALS_INVENT INGRESADAS
                                              /* UPDATE T049_DET_DOCUMENTO DET SET DET.F049_CANTIDAD_PTE_TOTAL_INVENT = (DET.F049_CANTIDAD_PTE_TOTAL_INVENT  - :NEW.F049_CANTIDAD_PTE_TOTAL_INVENT) 
												WHERE DET.F049_CODIGO 
												IN (
													SELECT DET.F049_CODIGO FROM T048_DOCUMENTO DOC SET 
													INNER JOIN T049_DET_DOCUMENTO DET ON DOC.F048_CODIGO = DET.F048_CODIGO 
													WHERE DOC.F012_TIPO_DOC = :NEW.F012_TIPO_DOC_RELACIONADO AND :NEW.F048_CONSECUTIVO = F048_CONSECUTIVO_RELACIONADO  AND DET.F004_CODIGO = :NEW.F004_CODIGO
													);
												   */                                            
            ...
            END IF;-- IF TIPO  = 'I'  THEN -- VALIDA INVENTARIO SOLO PARA LOS PRODUCTOS MAS NO PARA LOS SERVICIOS       
            END; 
